const BranchError = require('./branch-error');

module.exports.BranchError = BranchError;
...
function logger(level) {
	return function (code, reasons, meta) {
		if (code instanceof BranchError) {
			handleError(level, code);
			return code;
		}

		const error = new BranchError(code, reasons, meta);
