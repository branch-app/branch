...
const RecentMatchesCollectionName = "recent_matches"

func (recentMatches *RecentMatches) Upsert(db *mgo.Database, url string, validFor time.Duration) error {
	now := time.Now().UTC()

	// Check if we need to set CreatedAt and ID
	if !recentMatches.BranchResponse.ID.Valid() {
		recentMatches.BranchResponse.ID = bson.NewObjectId()
		recentMatches.BranchResponse.CreatedAt = now
	}

	recentMatches.BranchResponse.UpdatedAt = now
	recentMatches.BranchResponse.CacheInformation = sharedModels.NewCacheInformation(url, now, validFor)
	_, err := db.C(RecentMatchesCollectionName).UpsertId(recentMatches.BranchResponse.ID, recentMatches)
	return err
}

func RecentMatchesFindOne(db *mgo.Database, id bson.ObjectId) *RecentMatches {
	var document *RecentMatches
	err := db.C(RecentMatchesCollectionName).FindId(id).One(&document)
	switch {
	case err == nil:
		return document
