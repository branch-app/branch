...
func (a *app) GetPlayerCommendations(identityLookup xboxlive.IdentityLookup) (*response.Commendations, *log.E) {
	identity, err := a.xboxliveClient.GetIdentity(identityLookup)
	if err != nil {
		return nil, err
	}

	return a.waypointService.GetPlayerCommendations(*identity)
}
